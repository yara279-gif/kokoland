...
from kokoland.settings import EMAIL_HOST_USER
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            # body = render_to_string("user/wellcome_mail.html", {"user": user})

            # Send the email
            data = {
                "subject": f"Wellcome {user.first_name} to kokoland",
                "body": "wellcome ",  # Rendered HTML content
                "to_email": user.email,
            }
...
class addadmin(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            user = serializer.save()
            # Render email template
            # body = render_to_string("user/admin_mail.html", {"user": user})

            # Send the email
            data = {
                "subject": f"New Admin: Wellcome {user.first_name} to Ticketaya",
                "body":"Hello New Admin" ,  # Rendered HTML content
                "to_email": user.email,
            }
            Util.send_email(data)
